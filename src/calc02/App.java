...
public class App {
    public static void main(String[] args) {
        ...
        while (!keyword.equals("exit")) {
            ...
            keyword = sc.nextLine();
            switch (keyword) {
                case "view" -> calculator.viewResult();
                case "remove0" -> calculator.removeFirstResult();
                case "remove1" -> calculator.removeResult();
                case "clear" -> calculator.clearResult();
                case "exit" -> {
                    System.out.println("종료합니다.");
                    return;
                }
                default -> {
                        System.out.println("관련없는 명령어 입력으로 인해 새로운 연산을 시작합니다.");
                        keyword ="another";
                }
            }
            }
